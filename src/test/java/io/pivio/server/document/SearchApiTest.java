...
@ContextConfiguration(initializers = SearchApiTest.Initializer.class)
@Tag("integration-test")
public class SearchApiTest extends AbstractApiTestCase {

  @Container
  static final OpensearchContainer<?> opensearch =
      new OpensearchContainer<>("opensearchproject/opensearch:2.14.0").withStartupAttempts(5)
  ...
  public void setUpTestData() {
    ...
    postDocument(objectMapper.createObjectNode().put("id", "no1").put("name", "User Service")
        .put("short_name", "USS").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "lambda")
        .put("newfield1", "test1"));

    postDocument(objectMapper.createObjectNode().put("id", "no2").put("name", "Micro Service 2")
        .put("short_name", "MSS 2").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "lambda"));

    postDocument(objectMapper.createObjectNode().put("id", "no3").put("name", "Micro Service 3")
        .put("short_name", "MSS").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "lambda")
        .put("newfield1", "test2").put("newfield2", "test"));

    postDocument(objectMapper.createObjectNode().put("id", "no4").put("name", "Service 4")
        .put("short_name", "MSS").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "other")
        .put("newfield1", "test3"));

    postDocument(objectMapper.createObjectNode().put("id", "nestedObject").put("name", "Service 5")
        .put("short_name", "NOS").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "nestedTeam")
        .set("software_dependencies",
            objectMapper.createArrayNode()
                .add(objectMapper.createObjectNode().put("name", "de.websitename:file.jar").set(
                    "licenses",
                    objectMapper.createArrayNode()
                        .add(objectMapper.createObjectNode().put("key", "prop").put("fullName",
                            "Some Proprietary License"))))
                .add(objectMapper.createObjectNode().set("licenses",
                    objectMapper.createArrayNode()
                        .add(objectMapper.createObjectNode().put("key", "apl").put("fullName",
                            "Apache Public License"))
                        .add(objectMapper.createObjectNode().put("key", "gpl").put("fullName",
                            "GNU Public License"))))));

    postDocument(objectMapper.createObjectNode().put("id", "array").put("name", "Service Array")
        .put("short_name", "ARR").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "arrayTeam")
        .set("arrayfield", objectMapper.createArrayNode().add("a").add("c").add("bb").add("bd")));

    postDocument(objectMapper.createObjectNode().put("id", "array2").put("name", "Service Array 2")
        .put("short_name", "ARR2").put("type", "service")
        .put("description", randomStringGenerator.generate(20)).put("owner", "arrayTeam")
  ...
  public void search_returns_documents_matching_search_criteria() throws Exception {
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue).containsOnly("no2",
        "no3");
  }
  ...
  public void search_returns_documents_containing_only_requested_fields() throws Exception {
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue).containsOnly("no1",
        "no2", "no3");
    List<String> fieldNames = new ArrayList<>();
    searchResult.get(2).fieldNames().forEachRemaining(fieldNames::add);
    assertThat(fieldNames).containsOnly("id", "newfield1", "newfield2");
  }
  ...
  public void search_returns_documents_sorted_descending_by_short_name() throws Exception {
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue).containsExactly("no1",
        "no2", "no3");
  }

  @Test
  public void search_returns_documents_sorted_ascending_by_short_name_even_when_a_comma_follows_in_sort_parameter()
      throws Exception {
    // given
    ObjectNode ownerLambdaQuery = createOwnerLambdaQuery();
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue).containsExactly("no2",
        "no3", "no1");
  }

  @Test
  public void search_returns_documents_sorted_ascending_by_short_name_and_descending_by_owner()
      throws Exception {
    // given
    ObjectNode query = objectMapper.createObjectNode();
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue).containsExactly("no2",
        "array", "array2", "no4", "no3", "nestedObject", "no1");
  }
  ...
  public void search_returns_document_matching_nested_query() throws Exception {
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue)
        .containsExactly("nestedObject");
  }
  ...
  public void search_returns_all_documents_matching_data_in_array() throws Exception {
    ...
    // then
    assertThat(searchResult.findValues("id")).extracting(JsonNode::textValue)
        .containsExactlyInAnyOrder("array", "array2");
  }
  ...
  public void search_cannot_be_executed_when_fields_parameter_is_empty() throws Exception {
    ...
    ResponseEntity<JsonNode> responseEntity = restTemplate.getForEntity(
        "/document?query={query}&fields={fields}", JsonNode.class, query, emptyFields);
  ...
  private void assertThatSearchRequestWithSortParameterResultsInBadRequestResponse(String sortParam)
      throws Exception {
    // given
  ...
  private ArrayNode search(JsonNode queryParam, String fieldsParam, String sortParam)
      throws Exception {
    ...
    RequestEntity<Void> requestEntity = RequestEntity.get(URI.create(searchUrl))
        .header(CONTENT_TYPE, APPLICATION_JSON_VALUE).build();
  ...
  private ResponseEntity<JsonNode> search(String queryParam, String sortParam) throws Exception {
    return restTemplate.getForEntity("/document?query={query}&sort={sort}", JsonNode.class,
        URLEncoder.encode(queryParam, "UTF-8"), sortParam);
  }
}
